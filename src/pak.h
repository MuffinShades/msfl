...
/*
	...
	Written in C++ by James Weigand. December 2023.
...
extern "C" {
	...
	struct PakChunk {
		...
		int fSize = 0;
		int compressType = 0x00; //compressType = 0 -> no compression
		std::string fType = "";
		unsigned char* addHeaderInf = nullptr;
		int adhiLen = 0;
		int _fileIndex = -1;
		MZIP_SETTINGS compressionSettings;
		int resType = 0x00;
	};
	...
	public:
		...
		MSFL_EXP static int PakFilesInDirectory(std::string targetDir, std::string outputSrc, int compressionType = 0, MZIP_SETTINGS compressionSettings = MZIP_SETTINGS());
